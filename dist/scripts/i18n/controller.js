 ...
 */
define(['jquery', 'util', 'i18n/switch_reload', 'translate', 'i18n/config', 'cf/file_url', 'cf/site_config'], function ($, util, switch_reload, Translate, config, file_url, site_config) {
    //设置注册语言版本
    var langDic = {
        'en': 'English',
        'cn': '中文'
    ...
    var _switcherHtml = '<span class="language_switcher">' + langDic[defaultLanguage] + '</span>\n        <ul class="lang_switch_options">\n            ' + genOptions() + '\n        </ul>';

    var languageSwitch = function languageSwitch(lang) {
        if (langDic[lang]) {
    ...
    return {
        ...
        translateString: function translateString(template, data) {
            return translator.nodeString(util.generateHtml(template, data));
        },
        //使用翻译机封装util.getJSON接口，使其富有按照不同语言环境请求不同JSON数据的功能
        translateJSONGetter: function translateJSONGetter(name, cb) {
            return util.getJSON(file_url.filePath(defaultLanguage).data + name + '.json', function (data) {
                cb && cb(data);
            });
        },
        //语言切换按钮的html结构
        switcherHtml: function switcherHtml() {
            return _switcherHtml;
        },
        registerUpdateEvent: function registerUpdateEvent(functionName) {
