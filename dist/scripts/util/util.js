 ...
 */
define(['jquery', 'handlebars', 'cf/file_url'], function ($, Handlebars, url) {
    //输出html字符串
    ...
    return {
        extend: {
            Obj: function Obj(a, b) {
                for (var p in b) {
                    if (b.hasOwnProperty(p) && a && !a.hasOwnProperty(p)) a[p] = b[p];
                }return a;
            },
            Proto: function Proto(d, b) {
                for (var p in b) {
         ...
         * @return {*}
         */
        generateHtml: function generateHtml(template, data) {
            template = template && typeof template == 'string' ? template : typeof template == 'function' ? template() : template.nodeName && $(template).html();
            var compile = Handlebars.compile(template);
            return compile(data);
        },
        paramKeyFormat: function paramKeyFormat(key) {
            return key.toLowerCase().replace(/\s/g, '').replace(/\//g, '_');
        },
        showAllJSONData: function showAllJSONData() {
            return JSONDATA;
        },
        getJSON: function getJSON(path, cb) {
            var tdata = null;
            if (JSONDATA[path]) {
                tdata = JSONDATA[path];
                cb && cb(tdata);
        ...
        setPathName: function setPathName(pname) {
            window.location.pathname = pname;
        },
        getHashName: function getHashName() {
        ...
        //查询数据
        selectByID: function selectByID(data, id) {
            var target = null;
        ...
        //获取推荐数据
        getRecommends: function getRecommends(data, count, exceptID) {
            var _this = this;
            var tarD = [],
                gather = [];
            for (var l in data) {
        ...
        //设置缓存语言
        setLocalLanguage: function setLocalLanguage(lang) {
