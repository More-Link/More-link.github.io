 ...
 */
define(['jquery', 'handlebars', 'part/templates', 'util', 'cf/file_url', 'i18n/controller'], function ($, handlebars, templates, util, filePath, i18n_controller) {
    ...
    //清楚当前产品数据
    var currentColumnClear = function currentColumnClear() {
        for (var i = 0; i < product_params.length; i++) {
            product_tableTr.filter('.' + params_map[util.paramKeyFormat(product_params[i])]).children().eq(currentColumn).html('');
        }
    };

    //生成当前选中产品参数数据
    var generateCurrentProduct = function generateCurrentProduct(id, info_box) {
        ...
        for (var k in data.params) {
            util.extend.Obj(params, data.params[k]);
        } //遍历所有需要比对的参数
        for (var i = 0; i < product_params.length; i++) {
            product_tableTr.filter('.' + params_map[util.paramKeyFormat(product_params[i])]).children().eq(currentColumn).html(function () {
                var pvArr = params[product_params[i]],
                    _ht = '';
    ...
    var compareProWith = function compareProWith(id) {
        currentColumn = 1;
    ...
    var compareInitial = function compareInitial(compareID) {
        ...
        product_compare_table.on('click', '.item_placeholder', function () {
        ...
        }).on('click', '.item_options>li', function () {
            //选择操作
            var $this = $(this),
                p_id = $this.attr('value');
            $this.parent().data('p_id', p_id).toggleClass('active').siblings('.item_placeholder').text($this.text()).toggleClass('active');
        ...
        }).on('click', '.pro_item_add', function () {
            //新增选中操作
            ...
            var $this = $(this),
                proID = $this.siblings('.item_options').data('p_id');
            if (proID) {
                //显示切换
        ...
        }).on('click', '.pro_item_remove', function () {
            //移除当前操作
            var $this = $(this);
            //获取当前操作列
            currentColumn = $this.parents('td').index();
            //显示切换
            $this.parent().removeClass('active').siblings('.pro_item_selection').addClass('active').children('.item_placeholder').text(i18n_controller.config[i18n_controller.getLanguage()].page.compare.selectHolder).next().children('li').show();
            //清楚数据
            currentColumnClear();
        });
    ...
    var generateCompare = function generateCompare() {
        //生成对比参数列表
        util.getJSON('/data/product_params.json', function (param) {
            ...
            for (var i = 0; i < param.length; i++) {
                wrap += templates.product_compare.params_tr.replace(/\{\{param_name\}\}/g, param[i]).replace(/\{\{param\}\}/g, util.paramKeyFormat(param[i]));
    ...
    return {
         ...
         * @param compareID
         */
        initial: function initial(compareID) {
