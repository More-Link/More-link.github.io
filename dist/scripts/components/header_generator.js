...
define(['jquery', 'util', 'part/templates', 'cf/site_config', 'i18n/controller', 'i18n/config', 'part/nav_control'], function ($, util, templates, site_config, i18n_controller, i18n_config, nav_controller) {
  ...
  var generateHeader = function generateHeader() {
    ...
    i18n_controller.translateJSONGetter('products_all', function (data) {
      ...
      i18n_controller.translateJSONGetter('solutions_all', function (sdata) {
        ...
        //生成头部
        navBox.solution.children('li').remove().end().append(util.generateHtml(templates.header.solution, navigationData, true));
        navBox.product.children('li').remove().end().append(util.generateHtml(templates.header.product, navigationData, true));
  ...
  function generateNavList(data, base, category) {
    var baseLevel = $('<div><span class="nav_control" for="nav_solute_class_1">' + levelMap[base] + '</span>\n                            <ul class="nav_in_control nav_level_1 " id="' + base + '"/></div>'),
        //创建结构
    _base = baseLevel.find('#' + base); //获取ul
    for (var d = 0; d < data.length; d++) {
      //data为solution所有的对象,software&hardware,遍历2次
      var _data = data[d]; //software或hardware
      var index = 0,
          levels = _data.path.split('/');
      var matchChild = function matchChild(ul, level) {
        var _level = util.paramKeyFormat(level),
            _ul = null,
            _li = level != '' ? ul.children('#' + _level) : null;
